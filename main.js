...
const buildCartProducts = () => {
  ...
  for (let j = 0; j < cart.length; j++) {
    domString += `<div class="card mb-5 d-flex" style="max-width: 600px;" id="inCartProducts"">
...
const showWishlistPage = (e) => {
  ...
  document.getElementById("containerNoItemsInCart").style.display = "none";
...
const noItemsInCart = () => {
  if (cart.length === 0) {
    ...
    printToDom("containerCartProducts", buildNoItemsInCart());
    document.getElementById("containerOrderSummary").style.display = "none";
    document.getElementById("containerNoItemsInCart").style.display = "block";
  }
};
...
const commentBtnClick = (id) => {
  document
    .querySelector(`#comment-button${id}`)
